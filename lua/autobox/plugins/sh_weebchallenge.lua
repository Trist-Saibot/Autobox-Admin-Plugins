...
local PLUGIN = {}
PLUGIN.title = "Weeb Challenge"
PLUGIN.author = "Trist"
PLUGIN.description = "Quizzes Weebs"
PLUGIN.perm = "Weeb Quizzing"
PLUGIN.command = "wchallenge"

if (SERVER) then
    util.AddNetworkString("AAT_WeebChallenge")
    util.AddNetworkString("AAT_WeebChallengeEnd") --unused
    net.Receive("AAT_WeebChallenge",function(len,ply)
        ...
        if (correct) then
            ...
            autobox:CallPlugin("kick",ents.Create(""),ply,"failed the weeb challenge")
        end
    end)
end
...
if ( CLIENT ) then
    net.Receive("AAT_WeebChallenge",function()
        PLUGIN:StartWeebQuiz()
    end)
    net.Receive("AAT_WeebChallengeEnd",function()
...
function PLUGIN:StartWeebQuiz()
    ...
    wqback = Material("materials/autobox/weebquiz_background.png")

    function wq:Paint(w,h)
        surface.SetDrawColor(0,0,0)
        surface.DrawRect(0,0,w,25)
        surface.SetDrawColor(color_white)
        surface.SetMaterial(wqback)
