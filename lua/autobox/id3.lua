...
function id3.readtags(ifile)
	
	local rfile = file.Open(ifile, 'rb', "GAME")

	local position = rfile:Tell()
	...
	local function decodeID3v1(reader)
		...
		rfile:Seek('set', position)
		return info
	end
	
	local function readByte()
		local byte = assert(rfile:Read(1), 'Could not read byte.')
		return string.byte(byte)
	end
	local reader = {
		readStr = function(len)
			local str = assert(rfile:Read(len), 'Could not read '..len..'-byte string.')
			return unpad(str)
		end,
		readByte = readByte,
		...
		position = function() return rfile:Tell() end,
		skip = function(offset) rfile:Seek(rfile:Tell()+offset) end
	}
	
	-- try ID3v2
	rfile:Seek(0)
	local header = rfile:Read(3)
	if header == "ID3" then
		return decodeID3v2(reader)
	end
	
	-- try ID3v1
	rfile:Seek(rfile:Size()-128)
	header = rfile:Read(3)
	if header == "TAG" then
